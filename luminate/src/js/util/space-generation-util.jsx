...
/*
 *  Sangho's approach:
    *  1. Generate a list of requirements for each dimension
 */
export async function sanghoGenerateSpace(currBlockId, dimensions, numResponses, prompt, context){
    // generate a list of requirements for each dimension
    let {dimReqs, data} = genDimRequirements(dimensions, numResponses);
    const startTime = Date.now();
    let responses = [];
    console.log("dimReqs", dimReqs);
    // generate a response for each requirement in batches of 5
    const batchSize = 5;
    const batches = []
    for (let i = 0; i < dimReqs.length; i += batchSize){
        batches.push(dimReqs.slice(i, i + batchSize));
    }
    console.log("batches", batches);
    const batchPromises = batches.map(async (batch) => {
        // match each id to a requirement
        const batchIds = batch.map((req) => req["ID"]);
        const batchMessages = batch.map((req) => "Prompt: " + prompt + "\n" + DELIMITER + "\n" + "Requirements: " + req["Requirements"] + "\n" + DELIMITER + "\n");
        let responses = await sanghoGenerateResponse(batchIds, batchMessages);
        // check if the response is valid
        // if it is not valid, generate a new response
        for (let i=0; i< 5; i++){
            if (sanghoValidation(responses)){
                break;
            }
            responses = await sanghoGenerateResponse(batchIds, batchMessages);
        }
        // store the response in the data
        responses = responses.substring(responses.indexOf("{"), responses.lastIndexOf("}") + 1);
        responses = JSON.parse(responses);
        // use map to get the list of responses
        const batchResponses = Object.values(responses);
        console.log("batchResponses", batchResponses);
        // use map to get the list of summaries
        const batchSummariesPromises = await Promise.all(batchResponses.map(async (response) => {
            const summary = await abstraction(response);
            return summary;
        }));

        const batchSummaries = await Promise.all(batchSummariesPromises);
        // wait for all the summaries to be generated then store them in the data
        batchSummaries.forEach((summary, index) => {
            const id = batchIds[index];
            data[id]["Prompt"] = batchMessages[index];
            data[id]["Result"] = batchResponses[index];
            data[id]["Summary"] = summary["Summary"];
            data[id]["Keywords"] = summary["Key Words"];
            data[id]["Structure"] = summary["Structure"];
        });
        console.log("finished batch" + batchIds);
    ...
    // store the respones in the local storage
    print("put all data in blockID", currBlockId);
    DatabaseManager.putAllData(currBlockId, data);
    const setCurrBlockId = useCurrStore.getState().setCurrBlockId;
    setCurrBlockId(currBlockId);
    console.log("currBlockId", currBlockId);
...
async function sanghoGenerateResponse(batchIds, batchMessages){
    // concatenate the batchMessages into a single message in the format: id: message
    const batchPrompt = batchIds.map((id, index) => id + ": " + batchMessages[index]).join("\n");
    const message = `
    Prompt: ${batchPrompt}
    ####
    Requirement: 
    - limit each response to fewer than 150 words. 
    - put the response for each prompt in the following format:
    {
        "<id>": "<response>",
        "<id>": "<response>",
        ...
...
function sanghoValidation( response){
    try{
        // parse the response to get the id and response
        response = response.substring(response.indexOf("{"), response.lastIndexOf("}") + 1);
        const responseJson = JSON.parse(response);
